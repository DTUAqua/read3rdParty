...
read_in_cleaned_excel_without_elog <- function(file_path) {
  ...
  if ("Ark1" %in% sheet_names &&
    "Table 1" %in% sheet_names && "Ark2" %in% sheet_names) {
    sheet_name <- "Ark1"
  ...
  row_with_word <- which(apply(my_data, 1, function(x) {
    any(
      grepl(
        "Procent.+Prøve|Procent.+Prøven|Procent.+prøven|Procent.+prøve|Procent.+Prnve|Prooent.+Prøven|Procent.+Pr0ven",
        x
      )
    )
  }))
  ...
  final_df <- data.frame(lapply(final_df, function(x) {
    type.convert(x, as.is = TRUE)
  }))
  ignored_df <- data.frame(lapply(ignored_data, function(x) {
    type.convert(x, as.is = TRUE)
  }))
  ...
  combined_df <- combined_df %>%
    mutate(Dato = format((origin_date + days(as.numeric(
      Dato
    ))), format = "%d-%m-%Y"))
  ...
  ### Get species codes - the relation is now a part of the R package
